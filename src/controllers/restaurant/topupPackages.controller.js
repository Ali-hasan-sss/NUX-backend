...
exports.topUpUserBalanceByRestaurant = exports.deletePackage = exports.updatePackage = exports.getPackageById = exports.createPackage = exports.listPackages = void 0;
const client_1 = require("@prisma/client");
const response_1 = require("../../utils/response");
const check_restauran_owner_1 = require("../../utils/check_restauran-owner");
...
const listPackages = async (req, res) => {
    ...
    const check = userId ? await (0, check_restauran_owner_1.assertOwnerOrAdmin)(userId, restaurantId) : { ok: false };
    ...
    return (0, response_1.successResponse)(res, 'Packages fetched', items);
};
...
const createPackage = async (req, res) => {
    ...
    if (!restaurantId)
        return (0, response_1.errorResponse)(res, 'Restaurant ID is required', 400);
    const check = await (0, check_restauran_owner_1.assertOwnerOrAdmin)(userId, restaurantId);
    if (!check.ok)
        return (0, response_1.errorResponse)(res, check.msg, check.code);
    const { name, amount, bonus = 0, currency = 'EUR', description, isActive = true, isPublic = true, } = req.body;
    // Check current package count (maximum 5)
    const packageCount = await prisma.topUpPackage.count({ where: { restaurantId } });
    if (packageCount >= 5) {
        return (0, response_1.errorResponse)(res, 'Cannot create more than 5 packages. Delete an existing package first.', 400);
    }
    ...
    if (exists)
        return (0, response_1.errorResponse)(res, 'Package name already exists for this restaurant', 409);
    const created = await prisma.topUpPackage.create({
        data: { restaurantId, name, amount, bonus, currency, description, isActive, isPublic },
    });
    return (0, response_1.successResponse)(res, 'Package created', created, 201);
};
...
const getPackageById = async (req, res) => {
    ...
    if (!restaurantId)
        return (0, response_1.errorResponse)(res, 'Restaurant ID is required', 400);
    if (!id)
        return (0, response_1.errorResponse)(res, 'Package ID is required', 400);
    const packageId = parseInt(id, 10);
    if (isNaN(packageId))
        return (0, response_1.errorResponse)(res, 'Invalid Package ID', 400);
    const pkg = await prisma.topUpPackage.findFirst({ where: { id: packageId, restaurantId } });
    if (!pkg)
        return (0, response_1.errorResponse)(res, 'Package not found', 404);
    ...
    return (0, response_1.successResponse)(res, 'Package fetched', pkg);
};
...
const updatePackage = async (req, res) => {
    ...
    return (0, response_1.successResponse)(res, 'Package updated', updated);
};
...
const deletePackage = async (req, res) => {
    ...
    return (0, response_1.successResponse)(res, 'Package deleted', null);
};
 ...
 */
const topUpUserBalanceByRestaurant = async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        if (!userQr || !packageId) {
            return (0, response_1.errorResponse)(res, 'userQr and packageId are required', 400);
        }
        const user = await prisma.user.findUnique({ where: { qrCode: userQr } });
        if (!user) {
            return (0, response_1.errorResponse)(res, 'Invalid user QR code', 400);
        }
        ...
        if (!topUpPackage || topUpPackage.restaurantId !== restaurantId) {
            return (0, response_1.errorResponse)(res, 'Invalid package for this restaurant', 400);
        }
        ...
        return (0, response_1.successResponse)(res, 'Balance topped up successfully', balance);
    }
    catch (error) {
...
exports.topUpUserBalanceByRestaurant = topUpUserBalanceByRestaurant;
//# sourceMappingURL=topupPackages.controller.js.map